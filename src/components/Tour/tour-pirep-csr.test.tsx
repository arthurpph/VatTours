...
describe('TourPirepClientSide', () => {
    it('renders form fields and leg info', () => {
        ...
        expect(screen.getByRole('button', { name: /Enviar PIREP/i })).toBeInTheDocument();
    });
    ...
    it('shows error if callsign exceeds 7 characters', () => {
        ...
        expect(screen.getByText(/não pode exceder 7 caracteres/i)).toBeInTheDocument();
    });

    it('shows error if comment exceeds 100 characters', () => {
        render(<TourPirepClientSide leg={leg} />);
        const textareas = screen.getAllByRole('textbox');
        const textarea = textareas[1]; // Second textbox is the comment textarea
    ...
    // Note: This test is disabled because the component prevents submission when callsign is empty
    // by disabling the button rather than showing an error message
    it.skip('shows error if callsign is empty on submit', async () => {
        render(<TourPirepClientSide leg={leg} />);
        const button = screen.getByRole('button', { name: /Enviar PIREP/i });
        fireEvent.click(button);
        await waitFor(() => {
            expect(screen.getByText('Você precisa informar o indicativo de chamada.')).toBeInTheDocument();
        });
    });
    ...
    it('submits form and redirects on success', async () => {
        ...
        act(() => {
            fireEvent.change(screen.getByPlaceholderText('Ex: UAE406'), { target: { value: 'UAE406' } });
            const textareas = screen.getAllByRole('textbox');
            const textarea = textareas[1]; // Second textbox is the comment textarea
        ...
        await waitFor(() => {
            expect(screen.getByText(/PIREP enviado com sucesso/i)).toBeInTheDocument();
            expect(mockPush).toHaveBeenCalledWith(`/tours/${leg.tourId}?action=status`);
        });
    });
